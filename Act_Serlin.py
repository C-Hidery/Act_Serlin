...
class AdvancedChatAI:
    def __init__(self, name="Serlin", data_file="chat_data.json", language="zh"):
        ...
        self.autonomous_threshold = 300  # 学习多少回复后开始自主思考
    ...
    def deductive_reasoning(self, user_input):
        ...
        if self.language == "zh":
            logical_responses = [
                f"基于你提到的{', '.join(input_keywords[:2])}，似乎可以得出结论这是一个重要的话题。",
                f"考虑到你对{input_keywords[0]}的兴趣，我认为这与我们讨论过的更广泛主题有关。",
                f"从关键词{', '.join(input_keywords[:3])}来看，我推断这是一个值得进一步探索的复杂主题。"
            ]
        else:
            logical_responses = [
                f"Based on what you've said about {', '.join(input_keywords[:2])}, it seems reasonable to conclude that this is an important topic.",
                f"Considering your interest in {input_keywords[0]}, I think this relates to broader themes we've discussed.",
                f"From the keywords {', '.join(input_keywords[:3])}, I deduce this is a complex subject worth exploring further."
            ]
    ...
    def update_emotion(self, user_input, response_quality=0):
        """更新情感状态"""
        if self.language == "zh":
            positive_words = ["好", "开心", "高兴", "喜欢", "棒", "神奇", "美妙"]
            negative_words = ["坏", "难过", "生气", "讨厌", "糟糕", "可怕", "恐怖"]
        else:
            positive_words = ["good", "great", "happy", "love", "awesome", "amazing", "wonderful"]
...
def clear_screen():
    ...
    if os.name == 'nt':  # Windows
        os.system('cls')
    else:  # Linux, macOS 和其他Unix-like系统
        os.system('clear')
