...
class SerlinClient:
    ...
    def send_request(self, command, data=None):
        ...
        try:
            ...
            client_socket.send(request_length.to_bytes(4, byteorder='big'))
            #  ٷ       
            client_socket.send(request_json)
            print(f"Request sent ({request_length} bytes)")
            ...
            while bytes_received < response_length:
                chunk = client_socket.recv(min(4096, response_length - bytes_received))
                if not chunk:
                    break
                response_data += chunk
                bytes_received += len(chunk)
                print(f"Received {bytes_received}/{response_length} bytes")
    ...
    def chat(self, user_input):
        ...
        response = self.send_request('chat', {
    ...
    def get_system_status(self):
        ...
        response = self.send_request('get_system_status')
        return response
    
    def get_conversation_summary(self):
        """Get conversation summary from server"""
        response = self.send_request('get_conversation_summary')
        return response
    
    def add_training_data(self, questions, answers):
        """Add training data to server"""
        response = self.send_request('add_training_data', {
            'questions': questions,
            'answers': answers
        })
        return response
    ...
    def train_model(self, epochs=100, batch_size=4, learning_rate=0.0001, early_stopping=True):
        ...
        response = self.send_request('train', {
            'epochs': epochs,
    ...
    def validate_training_data(self):
        ...
        response = self.send_request('validate_training_data')
        return response
    
    def get_vocabulary_info(self):
        """Get vocabulary information from server"""
        response = self.send_request('get_vocabulary_info')
        return response
    
    def expand_vocabulary(self, words_text):
        """Expand vocabulary on server"""
        response = self.send_request('expand_vocabulary', {
    ...
    def reload_vocabulary(self):
        ...
        response = self.send_request('reload_vocabulary')
        return response
    
    def fix_model_vocab(self):
        """Fix model vocabulary synchronization on server"""
        response = self.send_request('fix_model_vocab')
        return response
    
    def reset_model(self):
        """Reset model on server"""
        response = self.send_request('reset_model')
        return response
    
    def export_conversation(self, filename=None):
        """Export conversation from server"""
        response = self.send_request('export_conversation', {
            'filename': filename
        })
        return response
    ...
    def load_model(self, path=None):
        ...
        response = self.send_request('load_model', {
            'path': path
        })
        return response
    ...
    def save_model(self, path=None):
        ...
        response = self.send_request('save_model', {
            'path': path
        })
        return response
    ...
    def create_training_template(self, file_path="training_template.json"):
        ...
        response = self.send_request('create_training_template', {
            'file_path': file_path
        })
        return response
    ...
    def batch_train_from_json(self, file_path):
        ...
        response = self.send_request('batch_train_from_json', {
            'file_path': file_path
        })
        return response
            ...
            if user_input.lower() in ['exit', 'quit']:
                ...
                if questions and answers:
                    ...
                    if add_result.get('status') == 'success':
                        print(f"Successfully added {len(questions)} training pairs")
                    else:
                        print(f"Error adding training data: {add_result.get('message')}")
                        continue
